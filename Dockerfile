...
COPY --from=builder /nextjs-starter/next.config.ts ./
COPY --from=builder /nextjs-starter/public ./public
COPY --from=builder /nextjs-starter/.next/standalone ./
COPY --from=builder /nextjs-starter/.next/static ./.next/static
EXPOSE 3000
CMD ["node", "server.js"]
